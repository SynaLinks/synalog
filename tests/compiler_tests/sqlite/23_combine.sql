WITH t_1_Items AS (SELECT * FROM (
    ...
    SELECT
      1 AS id,
      JSON_ARRAY('a', 'b') AS tags
   UNION ALL
  
    SELECT
      2 AS id,
      JSON_ARRAY('c') AS tags
   UNION ALL
  
    SELECT
      3 AS id,
      JSON_ARRAY('a', 'd', 'e') AS tags
...
t_2_FilteredTags AS (SELECT
  ARRAY_CONCAT_AGG(CASE WHEN (JSON_ARRAY_LENGTH(t_3_Items.tags) > 1) THEN t_3_Items.tags ELSE JSON_ARRAY() END) AS logica_value
FROM
