...
FROM
  UNNEST((SELECT ARRAY_AGG(x) FROM GENERATE_SERIES(0, 10 - 1) as x)) as x_2 ORDER BY col0),
t_0_P2 AS (SELECT
  x_5 AS col0
FROM
  UNNEST((SELECT ARRAY_AGG(x) FROM GENERATE_SERIES(0, 20 - 1) as x)) as x_5 LIMIT 5),
t_0_P3 AS (SELECT
  x_5 AS col0
FROM
